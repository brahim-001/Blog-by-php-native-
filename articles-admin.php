...
<body class="bg-light">
    ...
    <h1 style="text-align: center; margin-top:50px;">Articles</h1>
    <div class="container">
        <div class="mb-5 mt-3">
            Add a new article <button type="button" class="btn btn-secondary btn-sm"><a href="articleAdd.php">Add</a>
            </button>
        </div>
        <table class="table pb-5">
            ...
            <tbody>
                ...
                <?php
        $affiche = $conn->prepare("SELECT * from article ");
        $affiche->execute();
        ...
        foreach ($affiche->fetchAll() as $key => $article) :?>
                <tr>
                    ...
                    <td style="width: 300px"> <img src="uploads/<?= $article["image"] ?>" alt="" style="width:30%"></td>
                    <td> <?php echo $article["date"] ?> </td>
                    <td> <?php echo $article["id_categorie"] ?> </td>
                    <td> <?php echo $article["id_auteur"] ?> </td>

                    <td><a type="button" class="btn btn-secondary"
                            href="articleUpdate.php?id=<?= $article["id_article"] ?>">update</a>
                        <a href="articleDelete.php?id=<?= $article["id_article"] ?>" type="button"
